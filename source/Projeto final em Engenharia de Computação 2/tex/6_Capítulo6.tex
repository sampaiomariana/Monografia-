...
A fim de ter uma estruturação deste capítulo seguindo as ativades,tarefas e artefatos que foram desenvolvidos, este capítulo será apresentado de acordo com a estrutura do \acrfull{OpenUp}.

\section{Fase de concepção}

A fase de concepção possui uma sequência de atividades que podem ser seguidas no projeto em decorrências dessas atividades existem tarefas e nessas tarefas são gerados artefatos. Esse processo é proposto pelo \acrfull{OpenUp} e através dele permite que seja realizada a configuração do projeto. Sendo assim, as atividades que foram desenvolvidas na fase de concepção foram:
\begin{itemize}
...
A atividade de iniciar do projeto tem como objetivo inicializar o projeto e obter um acordo com as partes interessadas do projeto em relação ao escopo do projeto. Dentro dessa atividade foram desenvolvidas tarefas que são sugeridas pelo processo do \acrfull{OpenUp}. Sendo essas tarefas as seguintes:
...
foram produzidos artefatos e neles registrada informação acerca do projeto de software. Os artefatos construídos nessa fase foram os seguintes ~\cite{openup}:
\begin{itemize}
